...
// TestMoneyTransform_SetExchangeRate tests the SetExchangeRate method
// which allows setting custom exchange rates
func TestMoneyTransform_SetExchangeRate(t *testing.T) {
	tests := []struct {
		name     string
		currency CurrencyUnit
		rate     ExchangeRate
		...
		{
			name:     "set USD to EUR rate",
			currency: USD,
			rate:     ExchangeRate{Currency: CurrencyUnit("EUR"), Rate: 0.85},
		},
		{
			name:     "set VND to JPY rate",
			currency: VND,
			rate:     ExchangeRate{Currency: CurrencyUnit("JPY"), Rate: 0.0055},
		},
		{
			name:     "update existing rate",
			currency: USD,
			rate:     ExchangeRate{Currency: VND, Rate: 26000},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Act
			mt.SetExchangeRate(tt.currency, tt.rate)

			// Assert
			value, exists := mt.Load(tt.currency)
			if !exists {
				t.Errorf("After SetExchangeRate(), key %s does not exist", tt.currency)
			}
			storedRate, ok := value.(ExchangeRate)
			if !ok {
				t.Errorf("Value for key %s is not ExchangeRate type", tt.currency)
			} else {
				if storedRate.Currency != tt.rate.Currency {
					t.Errorf("Stored rate currency = %s; expected %s", storedRate.Currency, tt.rate.Currency)
				}
				if storedRate.Rate != tt.rate.Rate {
					t.Errorf("Stored rate = %f; expected %f", storedRate.Rate, tt.rate.Rate)
				}
			}
		})
	}
			...
			for i := 0; i < tt.goroutines; i++ {
				go func() {
					for j := 0; j < tt.operations; j++ {
						...
						mt.SetExchangeRate(USD, ExchangeRate{Currency: VND, Rate: 25000})
					}
					done <- true
				}()
			}
